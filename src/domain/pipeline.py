...
class DomainPipeline:
    ...
    def process(
        ...
        if candidate is None:
            if validation_result.destination == ValidationDestination.PERSONAL_CANDIDATE:
                ...
                return DomainProcessResult(
                    candidate_id="", raw_edge_id=edge.raw_edge_id, final_destination="personal"
                )
            else:
                self._stats["logged"] += 1
                return DomainProcessResult(
                    candidate_id="", raw_edge_id=edge.raw_edge_id, final_destination="log"
                )
        ...
        council_candidate = self.council.submit_candidate(
            ...
            source_document_id=edge.source_document_id or edge.fragment_id,
