...
class Fragment(BaseModel):
    ...
    page_number: Optional[int] = Field(default=None, description="원문 페이지 번호")
    chapter_title: Optional[str] = Field(default=None, description="원문 장 제목")
    section_title: Optional[str] = Field(default=None, description="원문 절 제목")
    block_type: str = Field(default="paragraph", description="원문 블록 유형")
...
class RawEdge(BaseModel):
    ...
    time_scope: TimeScope = Field(default=TimeScope.UNKNOWN)

    # 출처 추적
    fragment_id: str = Field(..., description="소속 fragment ID")
    fragment_text: Optional[str] = Field(default=None, description="원문 (검증용)")
    source_document_id: Optional[str] = Field(default=None, description="원천 문서 ID")
    source_type: Optional[str] = Field(default=None, description="원천 문서 유형")
    published_at: Optional[datetime] = Field(default=None, description="원천 문서 발행 시점")
    citation_start: Optional[int] = Field(default=None, description="원문 citation 시작 위치")
    citation_end: Optional[int] = Field(default=None, description="원문 citation 끝 위치")
    citation_page_number: Optional[int] = Field(default=None, description="citation 페이지 번호")
    citation_chapter_title: Optional[str] = Field(default=None, description="citation 장 제목")
    citation_section_title: Optional[str] = Field(default=None, description="citation 절 제목")
    source_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="원천 문서 메타데이터"
    )
...
class ExtractionResult(BaseModel):
    ...
    consolidated_relations: List[Dict[str, Any]] = Field(default_factory=list)
