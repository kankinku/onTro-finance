...
### 1. Domain Data (불변 · 뼈대)
- **성격**: 시스템의 "기본 지식 (Baseline Ontology)". 경제 개념, 고정된 정의. 파일 원본은 Read-only.
...
- **규칙**:
  ...
  - 시스템 시작 시 로드되며, baseline 파일 자체는 런타임에 변경되지 않음.
  - 런타임 승격은 `auto-approved` 또는 `council-approved` 관계에 한해 그래프 레이어에 제한적으로 반영될 수 있음.
...
**목적**: 보편적/불변 지식 (Baseline Ontology) 관리와 통제된 승격
**성격**: **Controlled Write** (baseline 파일은 read-only, 런타임 그래프 반영은 게이트 통과 시에만 허용)
**입력 조건**: Validation을 통과한 usable_edge
**책임**: 입력된 Edge가 Domain 지식과 일치하는지 확인하고, 애매한 후보를 council/offline 경로로 분기
...
#### 3.3 DynamicDomainUpdate
- **[현재]**: 런타임에 모든 후보를 직접 쓰지 않음.
- **처리**:
  - 명확한 후보는 auto-approve 후 Domain 그래프에 반영
  - 애매하거나 충돌하는 후보는 `Council Queue`로 전송
  - council 승인 또는 offline promotion 없이는 Domain 승격이 일어나지 않음
...
#### 3.6 DomainPipeline
- **순서**: Validated Edge -> Static Check -> Council Auto Assessment -> Auto-Approved만 Domain Update
- **핵심**:
  - high confidence/no conflict 후보만 Domain 반영
  - ambiguous/high-impact/conflicting 후보는 `council`로 분기
  - Personal Layer 저장은 `final_destination == "personal"`인 경우에만 수행
