...
from src.shared.models import ResolvedEntity, ResolutionMode
...
class TestQueryParser:
    ...
    def test_finance_query_prefers_driver_as_head(self):
        """민감도 질문에서는 원인 변수를 head로 둔다"""

        class StubNER:
            def extract(self, fragment_text, fragment_id, use_llm=False):
                return []
        ...
        parser = QueryParser(ner_student=StubNER(), entity_resolver=StubResolver())

        result = parser.parse("성장주는 금리 상승에 얼마나 민감한가?")

        assert result.query_type == QueryType.DIRECT_RELATION
        assert result.head_entity == "POLICY_RATE"
        assert result.tail_entity == "GROWTH_STOCKS"
