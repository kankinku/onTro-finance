...
class TestDatasetBuilder:
    ...
    def test_build_relation_dataset_from_council_logs(self):
        ...
        dataset = builder.build_dataset(TaskType.RELATION, include_domain=False, include_personal=False, include_logs=False)
...
class TestOfflineEvaluation:
    ...
    def test_build_dataset_logs_expected_adapter_failures_and_degrades(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr("src.learning.offline_runner.get_domain_kg_adapter", lambda: (_ for _ in ()).throw(RuntimeError("domain down")))
        monkeypatch.setattr("src.learning.offline_runner.get_personal_kg_adapter", lambda: (_ for _ in ()).throw(ValueError("personal down")))
    ...
    def test_build_dataset_does_not_swallow_unexpected_adapter_failures(self, monkeypatch):
        monkeypatch.setattr("src.learning.offline_runner.get_domain_kg_adapter", lambda: (_ for _ in ()).throw(TypeError("bug")))
